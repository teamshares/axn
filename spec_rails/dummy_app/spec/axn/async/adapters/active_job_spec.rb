...
RSpec.describe "Axn::Async with ActiveJob adapter" do
  ...
  let(:perform_enqueued) { -> { perform_enqueued_jobs } }
  let(:action_with_only_exhausted) { Actions::Async::ActiveJob::OnlyExhausted }
  let(:action_with_every_attempt) { Actions::Async::ActiveJob::EveryAttempt }
