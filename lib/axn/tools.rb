...
module Axn
  ...
  # return value.
  #
  # The other constants beneath it ARE adapter-facing, and an adapter names each directly: `Invoker`,
  # the sanctioned path for running an Axn as a tool (docs/reference/tool-invoker.md); `AdapterRoots`,
  # the opt-in directory-membership mixin an adapter extends onto its own config module; and
  # `AdapterSerialization`, the sibling mixin declaring the shared `reject_opaque_exposed_values`
  # setting and the resolve -> render / resolve -> map chain over it (docs/recipes/authoring-tool-adapters.md).
  #
