...
module Axn
  module Internal
    ...
    module SubfieldTree
      ...
      # mixed-union type on any route) — the SAME predicate emission gates on (Reflection::Schema.node_configs_block_nesting?),
      ...
      def blocking_ancestor?(node, key, carried = [])
        return true if Reflection::Schema.node_configs_block_nesting?(node.configs)
        return false unless node.children[key]&.implicit?

        colliding_shape_members(node, key, carried).any? { |m| !Reflection::Schema.nestable_as_object?(m) }
      end
      ...
      def merged_shape_members(node, key, carried)
        ...
        colliding_shape_members(node, key, carried).select { |m| Reflection::Schema.nestable_as_object?(m) }
      end

      # Every `shape:` member declared at `key` across the node's own configs AND the members carried from
      # a shallower hop — via Reflection::Schema.shape_members_at, the same locator emission uses, so the two sides
      # can't disagree on which members collide with the implicit child at `key`.
      def colliding_shape_members(node, key, carried)
        Reflection::Schema.shape_members_at(node.configs + carried, key)
      end
    end
  end
end
