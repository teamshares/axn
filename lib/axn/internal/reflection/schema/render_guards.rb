  ...
  module Internal
    module Reflection
      module Schema
        ...
        module RenderGuards
          # One guard Hash per action class, keyed by wire key (a String — `Values.canonical_wire_key`, the
          # same canonicalization `serialize_exposed` keys its own per-field walk with) — or nil when no
          # position in this class's output is member-derived at all. THE one entry point; everything below
          # is a helper for it, in the same style Contents/Nestability keep their own helpers unenforced-but-
          # internal rather than `private_class_method`-listed.
          def output_render_guards(field_configs)
            guards = field_configs.each_with_object({}) do |config, acc|
              guard = field_render_guard(config)
              next unless guard

              acc[Axn::Internal::Reflection::Values.canonical_wire_key(config.field)] = guard
            end
            guards.empty? ? nil : guards
          end
          ...
          def field_render_guard(config, ancestry: nil)
            ...
            if in_items
              combine_render_guard(items: of ? contents_render_guard(of, ancestry) : nil)
            elsif ::Hash.equal?(of_container(validations))
              hash_field_render_guard(shape, validations, of, ancestry)
            elsif shape
              shape_field_render_guard(shape, validations, ancestry)
            end
          end
          ...
          def shape_field_render_guard(shape, validations, ancestry)
            return nil unless shape_serializes_to_object?(validations)

            classes = guarded_render_classes(Axn::Internal::ShapeGraph.type_tokens(validations.dig(:type, :klass)))
            combine_render_guard(classes:, members: member_render_guards(shape[:members], ancestry))
          end
          ...
          def hash_field_render_guard(shape, validations, of_bag, ancestry)
            values = of_bag ? map_values_render_guard(of_bag, ancestry) : nil
            members = shape && shape_serializes_to_object?(validations) ? member_render_guards(shape[:members], ancestry) : nil
          ...
          def contents_render_guard(bag, ancestry)
            ...
            classes =
              if bag[:klass].nil?
                ...
                contents_klass_render_classes(bag[:klass])
              end
            members = overlay ? member_render_guards(shape[:members], ancestry) : nil
            ...
            unless nil.equal?(inner)
              guard_contents_descent(inner, ancestry, edge: :of) do |child|
                if Axn::Internal::ShapeGraph.map_bag?(inner)
                  values = map_values_render_guard(inner, child)
                else
                  items = contents_render_guard(inner, child)
                end
              end
            end
          ...
          def map_values_render_guard(bag, ancestry)
            axis = Axn::Internal::ShapeGraph.hash_or_nil(bag[:values])
            return contents_render_guard(axis, ancestry) unless nil.equal?(axis)

            classes = contents_klass_render_classes(bag[:values])
          ...
          def contents_klass_render_classes(klass)
          ...
          def build_member_render_guards(members, ancestry)
            named_members(members).each_with_object({}) do |(member, name), acc|
              acc[Axn::Internal::Reflection::Values.canonical_wire_key(name.to_sym)] = field_render_guard(member, ancestry:)
            end
          end
