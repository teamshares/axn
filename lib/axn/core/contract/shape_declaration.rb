  ...
  module Core
    module Contract
      module ShapeDeclaration
        ...
        #
        # Also stands down on a shape that ANSWERS a missing key from a Hash default (`Hash.new(…)` or a
        # `default_proc`) — `copy_entries` below is entry-wise, so a `members:`/`container:` the shape only
        # ANSWERS is invisible to it, and every REAL entry beside it (however few) reads as unknown. That is
        # `_walk_shape_graph!`'s defect to report (`ShapeGraph.reject_defaulting_option_container!`), and it
        # already runs on every route this guard does — firing here first would report the wrong defect
        # (`shape: does not support "container"`, naming a String key that IS real, over the actual problem).
        #
        ...
        def _reject_unknown_shape_keys!(carrier)
          return unless Internal::ShapeGraph.carries_key?(carrier, :shape)

          shape = Internal::ShapeGraph.hash_or_nil(carrier[:shape])
          return if nil.equal?(shape) || Internal::ShapeGraph.supplies_default?(shape)
        ...
        def _snapshot_inner_shape!(bag, walk, allowance, fields:, position:, via:, via_name:)
          ...
          _reject_unknown_shape_keys!(bag)
