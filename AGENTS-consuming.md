...
`OutboundValidationError`. (A `done!` inside a `default:`/`preprocess:` — inbound, or an `exposes`
`default:` — settles as success immediately, and outbound validation never runs.)
