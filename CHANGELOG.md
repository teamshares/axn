...
* [BREAKING] A self-referential `shape:` graph — a raw `shape: { members: [...] }` whose nested shape is reachable from inside itself — now raises `ArgumentError` at class definition, naming the member that closes the loop. Previously it overflowed the stack while the class was being defined, and `SystemStackError` is outside `StandardError`, so no rescue could settle it. Relatedly, the member-name and `user_facing:` declaration guards over a raw `shape:` kwarg now decide types and reader membership from the real class and method table, so a member (or shape Hash) that overrides `is_a?`/`respond_to?`/`==` can no longer skip a guard that reflection's own walk still consumes, nor replace its declaration error with an exception of its own.
