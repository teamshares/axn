...
* [INTERNAL] Gems generated by `bin/new-gem` are publish-ready: the gemspec pins `allowed_push_host` to `https://rubygems.org` so `rake release`'s `gem push` can't be redirected by a stray `RUBYGEMS_HOST` or a misconfigured remote (`rubygems_mfa_required` was already set), and the generated README documents the release flow — bump + changelog, **commit** (bundler's `guard_clean` aborts a release against a dirty tree, staged or unstaged), then `rake release`. The release gate itself is unchanged: `build.enhance` already ran the full verify/default suite before anything is tagged or pushed.
