...
## Unreleased
* [FEAT] `expects … on:` now accepts a **dotted path** to reach a deeply-nested parent (e.g. `expects :zip, on: "address.billing", type: String` validates `address[:billing][:zip]` and defines a flat `zip` reader). The root segment must be a declared field/subfield. `default:`/`preprocess:` combined with a dotted `on:` raise `ArgumentError` (writing into an arbitrary nested path isn't supported yet); single-key `on:` is unchanged.
