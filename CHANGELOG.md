...
* [BREAKING] Two members at the same nesting level of one shape may no longer share a name, or two names that render as the same JSON property — a member nested inside a member block is a property of a different JSON object, so it doesn't collide with one of the same name at another level. Declaring the same member twice previously built both members and kept only the last in the reflected schema, discarding the first with no signal.
