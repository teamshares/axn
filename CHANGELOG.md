...
### Downstream gem tooling

* [INTERNAL] `bin/new-gem NAME` (PRO-2949) scaffolds a new downstream axn-consuming gem so a fresh gem starts conformant instead of being reverse-engineered from an existing one. It shells out to `bundle gem NAME` for the generic baseline, then lays axn core's canonical delta on top: `bin/refresh`, the release-gated `Rakefile`, canonical `.rspec`/`.gitignore`/`.github` (CI on `ci.yml`, Ruby 3.2–3.4 matrix, CODEOWNERS, renovate), the canonical gemspec (full reject-list, `bindir`, axn dependency), the `axn` main-branch Gemfile pin, an `extend Axn::Configurable` + `config_namespace` + `deprecator` entry stub (with the hyphenated `lib/<name>.rb` shim bundler 2.5 no longer emits), the canonical `spec/spec_helper.rb`, and `AGENTS.md` + a `CLAUDE.md` symlink. When run interactively it prompts for a one-line summary (fills the gemspec `summary`/`description` and the README), falling back to a gem-name-aware stub non-interactively. Standardizes the drift observed across `slack_sender`/`data_shifter`/`axn-mcp`/`axn-ruby_llm`. Dev-only — lives under `bin/`, absent from the packaged gem.
* [INTERNAL] `.rubocop.yml` now ships in `spec.files` so internally-built downstream gems can `inherit_gem: { axn: ".rubocop.yml" }` (what `bin/new-gem` wires) and carry only their per-gem deltas. Deliberately an internal convention, not a documented public API — `.rubocop_todo.yml` and other `.rubocop*` files stay excluded.
