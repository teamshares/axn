...
* [FEAT] `step` accepts `if:` / `unless:` for conditional execution — a Proc (instance-`exec`'d on the parent) or a Symbol naming a parent method, combinable (the step runs only if `if:` is truthy **and** `unless:` is falsey), evaluated immediately before the step would run. A skipped step exposes nothing and cannot fail; later steps still run. Conditions read the parent's `expects` inputs and its own methods (not values exposed by earlier steps — those aren't readable on the parent mid-run). A non-Symbol/non-callable condition raises at declaration.
