...
* [FIX] An exception from outside `StandardError` — a `SystemStackError` from runaway recursion, a `NotImplementedError` from an unfinished method — now settles as the `exception` outcome it is before being re-raised: `result.outcome`/`ok?` are honest, `on_error` and `on_exception` fire per the documented superset contract, and the global handler is notified once (at the innermost action, with the `axn_stack` breadcrumb). Previously nothing rescued these, so the run settled as nothing at all — `outcome` read `success`, the completion line said so, and a user's own runaway recursion was invisible to error tracking. `fails_on` is deliberately not consulted, so a matcher broad enough to catch one (`fails_on Exception`) can't turn an abort into a returned failure result, and the original exception object is re-raised unwrapped. Exceptions that say nothing about the action are passed through completely untouched — no outcome, no callbacks, no report, no log line: `SignalException` (so `Interrupt`, and `Sidekiq::Shutdown`), `SystemExit`, and `Timeout::ExitException`, which must reach the enclosing `Timeout.timeout` intact for the timeout to fire.
...
* [FEAT] `Axn::Extensions.best_effort` now swallows `StandardError` plus a curated allowlist of non-`StandardError` classes (`Axn::Extensions::SWALLOWABLE_BEYOND_STANDARD_ERROR`, currently `SystemStackError`), so a side channel — a log line, a span update, an error report — can never be what takes down the call it describes. The allowlist is deliberately tiny rather than "everything that isn't a signal": the non-`StandardError` set in a live process is open-ended, and members like `Timeout::ExitException` and `ActiveSupport::ErrorReporter::UnexpectedError` exist precisely so nothing swallows them (eating the former would make an enclosing `Timeout.timeout` silently not fire). Pass `standard_errors_only: true` when the block's return value or side effect feeds the call's real behavior rather than observing it — resolving a `model:` record, deciding whether an error handler applies, filtering which records enqueue — so a runaway user callable surfaces its own stack instead of reading as a benign absence.
