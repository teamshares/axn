...
* [FEAT] A declared base `error "…"` now prefixes the action's specific failure *reasons* — conditional `error … if:`/`unless:`, entries explicitly marked `prefixed: true`, and `fail!` messages — rendered as `"<base><delimiter><reason>"`. **Conditionality sets an entry's role, not whether it's a literal or a block:** an *unconditional* `error`/`success` (string **or** block — they behave identically) is the **base headline**; a *conditional* one is a prefixed reason. `prefixed:` is the explicit override — `prefixed: true` promotes an unconditional entry to a prefixed reason (e.g. `error(prefixed: true, &:message)` for an always-on detail under the base), `prefixed: false` makes a conditional reason render standalone. Prefixing is **gated by a declared base** (no base ⇒ reasons render standalone). The join string is `delimiter:` on the base (default `": "`; `delimiter:` on a reason raises at declaration). When multiple unconditional headlines are declared, the most-recent wins (a headline whose block raises/blanks falls back to an earlier one). `success`/`done!` mirror all of this. Note: `result.error` returns the **prefixed** presentation string — `Axn::Failure#message` (seen when rescuing from `call!`) carries the raw reason without the prefix. **Behavior change for existing actions:** an action pairing an unconditional base with a *conditional* entry will see the conditional message gain the base prefix by default (opt out with `prefixed: false`); an *unconditional* block is now a headline (replacing, not prefixed) unless you add `prefixed: true`.
