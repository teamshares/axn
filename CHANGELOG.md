...
* [INTERNAL] Reusable `gem-ci.yml` check names are now stable and self-describing: the caller templates pin the calling job to `name: CI`, and the main job is `Main Specs (Ruby X)` (was `Specs`), matching core's own workflow and reflecting that the default rake task runs RuboCop too. **A gem adopting the reusable workflow must set its branch-protection required checks to `CI / Main Specs (Ruby 3.2|3.3|3.4)` and `CI / Rails Specs (…)`** — GitHub prefixes a reusable workflow's jobs with the caller's job name, so the bare job name never reports and a ruleset pinned to it blocks every PR indefinitely.
