...
* [FEAT] `Axn::Extensions.best_effort` now swallows `StandardError` plus `Axn::Extensions::SWALLOWABLE_BEYOND_STANDARD_ERROR` (`SystemStackError`, `ScriptError`), so a side channel — a log line, a span update, an error report — can never be what takes down the call it describes. That one allowlist is also what `Core::Executor` may settle onto a result, so there is a single answer to what axn will ever swallow, exposed as `Axn::Extensions.swallowable?`. It is an allowlist rather than a denylist of "everything that isn't a signal" because the non-`StandardError` set in a live process is open-ended — any gem can define a direct `Exception` subclass, and several (`Timeout::ExitException`, `ActiveSupport::ErrorReporter::UnexpectedError`) exist precisely so that nothing swallows them. Pass `standard_errors_only: true` when the block's return value or side effect feeds the call's real behavior rather than observing it — resolving a `model:` record, deciding whether an error handler applies, filtering which records enqueue — so a runaway user callable surfaces its own stack instead of reading as a benign absence.
