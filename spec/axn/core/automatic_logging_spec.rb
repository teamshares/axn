...
RSpec.describe Axn::Core::AutomaticLogging do
  ...
  describe "log separators with nested actions" do
    ...
    before do
      ...
      allow(Axn::Util::BackgroundJob).to receive(:running_in_console?).and_return(false)
      allow(Axn::Util::BackgroundJob).to receive(:running_in_background?).and_return(false)
