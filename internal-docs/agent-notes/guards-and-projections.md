...
- **The first two cuts tried to WITNESS the branch, and a witness does not terminate.** A stand-down is a claim that something PASSES, so it owes every check in the declaration a pass — and three consecutive review rounds each found another check it had not been asked. Round one: the author's own `length:` ceiling (`{minimum: 1, maximum: 3}` accepted, because 5 clears the minimum while `false` violates the maximum), and `{is: 2}` through the other spelling. Round two: a closed `inclusion:` set. A single sweep of the entry space then found four more in one go — `exclusion:`, `format:`, `numericality:`, `comparison:`, `acceptance:` — with a custom `validate:` unanswerable in principle. Deciding one value against every validator in a declaration is a satisfiability solver, and building one inside a guard is the same unbounded treadmill `Internal::NativeMethods` exists to refuse.

  **The rule that terminates asks about the declared TYPE, which is finite, not about a value.** Is every branch one the ceiling bounds — `blank_is_empty_class?`, or a token the `absence:` rejects outright (no `true`, Integer or Float is blank), or neither, in which case stand down. It costs three narrow refusals the witness version got right and cannot acquire a new hole from a validator nobody thought of. Two guardrails the exercise produced, both worth reusing: keep a token whose values the check rejects OUTRIGHT in the "bounded" set, or the type-level question collapses into "any union disables the rule"; and **scope the stand-down to the ceiling it is about** — dropping the `absence_bounds_size?` gate made it suppress `length:` contradictions too, which the pre-existing `type: String` example caught immediately.

- **Probe for UNDER-refusals after adding a stand-down.** The fix's own direction is the one the existing suite structurally cannot see: every example there asserts a refusal or an acceptance that already held. Product-probe {every bound spelling} × {every other validator entry} and look for cells where nothing passes and nothing is refused. Both of the rounds above were findable that way before review, and the third sweep is what turned "one more check" into "this design does not terminate".
...
**Kali's ruling on that pair (2026-08-26), and it settles the general rule as well as this case:** a class lying about itself is non-standard behaviour and not the gem's problem — so such a value is not counted as the witness that would make a declaration satisfiable, and nothing here hardens against it. But that line does **not** license refusing what an honest value can satisfy, and the priority is explicit: *"better NOT to provide that extra affordance than to incorrectly prevent them from writing actually-satisfiable code."* A declaration-time rule is a convenience — it tells an author now what they would otherwise learn on the first call — and a convenience is worth less than the freedom to write working code.

So the ordering to apply, in this order: is there an HONEST value that satisfies the declaration? If yes, stand down, whatever the projection looks like (that is why the union branch gives up three correct refusals to keep `false`). If the only witness is a value that lies about its own measurement, the rule stands. **The check that matters is a measurement, not an argument** — across 3072 product cells against a spread holding sizes 0..6 of every container, zero refusals are satisfied by an honest value.

Two facts worth keeping about the alternative, neither of which is the reason for the choice: honouring the lying value costs **22 of 76 refusals**, the two spellings the rule exists for included (`absence:` on a typed field, `length: { maximum: 0 }`), since every size a declaration bounds can be answered by a value that measures itself; and the declaration would then emit `{type: "array", enum: [[]], minItems: 3}`, a node no document satisfies. Earlier drafts of this note led with that second point. It is support, not the argument — leading with a projection rule invites exactly the trade Kali's ruling forecloses.
