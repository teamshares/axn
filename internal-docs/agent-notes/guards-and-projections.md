...
- **A witness must satisfy the WHOLE declaration, and the first cut only checked the floor.** The next review round reported it: `length: { minimum: 1, maximum: 3 }` was accepted because 5 clears the minimum, while `false` violates the maximum and nothing else can pass. `length: { is: 2 }` was the same hole through the other spelling. Note where the ceiling has to come from — the `length:` entry's own, NOT the `maximum` the comparison holds, since that one IS the absence-derived 0 the witness is being weighed against. **A stand-down is a claim that something passes, so it owes every check a pass, not the one that motivated it.** The countermeasure that would have caught it before review is the same one that keeps working here: after adding a stand-down, product-probe {every bound spelling} × {the shapes it now admits} and look for UNDER-refusals, not just over-refusals — the fix's own direction is the one the existing suite cannot see.
