...
| `readers: false` is subfield-only (top-level rejects it) | **Keep** — the reverse direction is structural: a readerless top-level field would break `resolve_parent`/`inputs` |
