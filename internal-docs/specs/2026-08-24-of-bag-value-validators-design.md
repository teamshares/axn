...
Surviving the string form has a second half on OUTPUT, where the key axn validated and the key axn serializes are two different objects. Two keywords ask about the key OBJECT rather than the property name it becomes, and both are wrong when the two come apart. `length:` is measured by ActiveModel on the object's `#length` while the emitted `minLength`/`maxLength` measure the property name — a key whose `#length` counts segments serializes to `"a/b"`, runtime 2 and wire 3. `inclusion:` is matched by Ruby `==`, which can identify values with different wire forms — `1 == 1.0`, so a `{ in: [1] }` axis accepts a `1.0` key serializing to `"1.0"` while the emitted set holds only `"1"`. Both reduce to one question — is the key its own wire form — so one gate answers both rather than a keyword-by-keyword table that has to be re-argued for each keyword added. It admits String, a String subclass and Symbol, and withholds the projection for every other token, an absent `klass:` included, since the keys may then be anything.

The two key-axis gates run in OPPOSITE ancestry directions, which is why they are two predicates and not one. Inbound asks whether a String could REACH the axis (`String.ancestors.include?(token)`) — `klass: Object` answers yes, correctly, since a JSON client really can satisfy it. Outbound asks whether every key the axis admits IS a String (`token.ancestors.include?(String)`) — `klass: Object` answers no, equally correctly, since it admits the segmented key above. Sharing one predicate between them was a bug: the reachability answer kept an outbound bound on an axis promising nothing about its keys. `format:` is exempt from both by construction rather than by omission: ActiveModel matches `value.to_s` and the serializer writes that same `to_s`, so its subject is the wire form already.

What these gates reason from is the DECLARED class, which is the boundary as well as the mechanism. A class that misreports itself defeats them — a `String` subclass overriding `#length` to disagree with its own bytes, or `#to_s` to return a different string, passes the runtime check and serializes to something the emitted keyword rejects. That is out of scope per AGENTS.md's rule on adversarial objects, and it is not fixable at this layer in any case: reflection never sees a runtime key, so defending against it means standing the projection down for every axis that names `String` at all — the feature, not an edge of it. The line is whether the class misreports itself: a key whose `#length` counts path segments is a legitimate value object and is what the gate is FOR, while a `String` subclass whose `#length` disagrees with its own characters is not. Audited alongside: an honest String subclass, an astral character, a combining sequence and a frozen literal all agree between runtime and emitted node, and a key whose bytes have no UTF-8 form raises `Serialization::UnserializableValue` rather than serializing at all.
