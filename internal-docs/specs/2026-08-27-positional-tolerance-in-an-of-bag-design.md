...
| `except_on:` (AM 7.2) | **declares cleanly, inert** | refused as unknown | refused | refused |
| `except_on:` (AM 8.x) | **declares cleanly, inert** | **declares cleanly, inert** | refused | refused |
...
* `except_on:` — a hole this settlement closes, and the only one whose current shape depends on the consumer's ActiveModel version. The gemspec is `activemodel >= 7.2`, and `except_on` joined AM's shared-option list after 7.2: under the locked 7.2.2.2 `_validates_default_keys` is `[:if, :unless, :on, :allow_blank, :allow_nil, :strict]`, under 8.x it also carries `:except_on`. Measured under `bundle exec` on both, `except_on:` declares cleanly and is inert at a field entry on either version; in an element bag it is refused as an *unknown key* on 7.2 and declares cleanly on 8.x; at an axis it is refused on both, by different routes. It is AM's other context option and axn has no contexts, so it is inert — but inert in the opposite direction to `on:`.

  Two consequences for the implementation. The predicate must key on the KEY's presence rather than on AM's shared-option list, or the guard's coverage would silently vary by the consumer's AM version. And `except_on` must be added explicitly to `OF_OPTION_KEYS` (and to `UNADVERTISED_OF_KEYS` beside `on:`/`strict:`), because `_reject_unknown_of_keys!` runs ahead of the dedicated guards — without the explicit entry, 7.2 reports "unknown key" and the message that names the real problem is unreachable at a bag position. AM installs it as `unless: -> { Array(options[:except_on]).include?(validation_context) }`, and axn calls `valid?` with no context, so `[:y].include?(nil)` is false and the entry **always** runs. The exclusion excludes nothing on any call. Refused at every position, structured as its own predicate/refusal pair rather than by widening `entry_context_scoped?` — the message has to name the opposite direction from `on:`'s "that check runs on no call at all", and a shared predicate feeding two messages is the drift these pairs exist to prevent. It mirrors `strict:` exactly: `Base.entry_declares_except_on?` beside `entry_declares_strict?`, `_reject_validator_except_on!` beside `_reject_strict_validation!` for a field's entries, `_reject_inner_contract_except_on!` beside `_reject_inner_contract_strict!` for a bag at any position.
