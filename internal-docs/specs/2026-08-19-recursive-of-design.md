...
**One carve-out on row 1: an element class of `::Array` folds to `ANY_CONTAINER`, not to `::Array`.** `container: Array` is not a gate at that key — `ShapeValidator` reads it as "distribute over the elements" — so storing it would read the members one level too deep and reject valid input. There is no way to spell "read members off this Array element" while `::Array` means distribute, so the folded position is left ungated; the bag's own `klass:` still enforces that the element IS an Array. The hand-written twin (`of: { klass: Array, shape: … }` with no distributing block) is **refused at declaration**, pointing at `of: { klass: Array, of: { shape: … } }` — the recursion this change adds, which emits `items.items.properties` correctly. An earlier draft left the twin legal and booked the divergence to PRO-3192; the final review found it published nothing of what it enforced, so it is closed here and the bag means one thing at every position.
...
**Shared options: inert on a map AXIS, live everywhere else.** An earlier draft of this spec said they are inert inside any nested bag. That was measured wrong during Task 5 and the truth is sharper. A nested ELEMENT bag becomes the next level's ActiveModel entry (`inner_contract_validations` hands `bag[:of]` over verbatim), so `if: -> { false }` there really does suppress the nested check — they are live, not ignored. An AXIS bag is never an AM entry (`axis_contract` reads only `klass:`/`message:`/`of:`/`shape:`), so there and only there they were unenforced-by-omission, and Task 5 refuses them at that one position (`AXIS_INERT_OPTION_KEYS`). They stay admitted everywhere else because axn's own tolerance push WRITES `allow_blank:`/`allow_nil:` into the top-level bag — a blanket ban would refuse `of: Integer, optional: true`. The narrower residue that does still stand: `allow_nil:`/`allow_blank:` govern whether the whole field may be absent and never make an individual element optional, per `docs/reference/class.md:184`.
...
A second narrowing of the same shape: **a `klass:` naming something that is not a type at all now raises at declaration, at every bag position.** `of: false` expands to `{ klass: false }`, which NAMES something, so the constrains-nothing guard passes it and `TypeValidator.value_matches?` reaches `value.is_a?(false)` — a bare `TypeError: class or module required` on the first call carrying a non-empty container, naming neither the field nor the option and reported as an internal error. The bare-axis spelling (`of: { values: false }`) was refused at declaration by PRO-3165; every BAG spelling of it declared cleanly, because `_reject_unsupported_map_axis!` deliberately skips a Hash-valued axis — that axis is a bag, judged by the bag grammar — and the bag grammar never asked the question. So the check belongs where every bag is judged (`_check_inner_contract_bag!`), which closes it at all six positions at once: an Array's element, either axis of a map, a bag nested inside either, and a bag under a shape member's own `of:`. The predicate is the axis guard's own (`_supported_type_token?`), extracted rather than mirrored, so the two cannot drift about what a type is; the message names `klass:` rather than the axis, since `klass:` is what there is to edit wherever it was written. Two spellings are left to the guards that already own them: an empty or absent `klass:` keeps the constrains-nothing message, and a bag carrying its own `of:` keeps `of: requires klass: Array or Hash`, which is the strictly narrower fix. Extracting the predicate also closed the axis guard's own residue: it searched with `find`, whose answer for "found nil" is its answer for "found nothing", so `of: { values: [String, nil] }` slipped past a check that had refused `values: nil` since PRO-3165. Searched by index now.
