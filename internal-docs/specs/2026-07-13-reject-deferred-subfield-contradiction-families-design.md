...
- A **Proc default counts as a rescue** (`usable_default?` returns true for a Proc): it applies at runtime; a raising Proc is unknowable, and rejection must be reserved for *provably* dead declarations. Strict mode keeps excluding Procs (schema stays safe-direction).
...
- Consumer sweep before merge: grep os-app, axn-mcp, axn-ruby_llm, data_shifter, slack_sender for declarations hitting the new rejections or relying on default-mutation.
