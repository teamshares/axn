...
### 5. Validation: eager when the namespace is registered, deferred otherwise

Three write paths, validated at whichever moment the schema is knowable — without a *core-owned* registry (rejected below). The seam is a **per-class** namespace→source map that each source populates from its own `.overrides` include hook (the same inclusion that installs the accessors — not core learning about gems):

- **Flat accessor** (subclass gem): generated by the gem's `setting`, knows the schema, validates at write. Unchanged.
- **`configure(:ns)` writer, namespace registered** (the class included that source's `.overrides`, so the schema is present — always true for `:core`): the writer looks up the source and validates the setter name and value eagerly, so a typo fails at class definition like the flat setter would.
- **`configure(:ns)` writer, namespace unregistered** (adapter not loaded / overrides not included): stores blindly. The owning adapter validates its slice when it *reads* — `resolve_override_for` runs `setting.validate!` on the resolved override — so a bad value surfaces at wrap/invocation time; for an adapter never loaded, the value simply never resolves.

The layering: include an adapter's `.overrides` → eager validation (via flat accessors *or* `configure(:ns)`); don't → tolerant `configure(:ns)`, deferred to read. `:core`'s overrides are always included on every action, so Axn's own settings are always validated eagerly.
...
## Rejected alternative: a core-owned global registry

An adapter could register its namespace + schema with **core** on load, so `configure(:mcp)` validates eagerly process-wide whenever axn-mcp is present. Rejected: a core-owned registry must track which gems exist, coupling core to its satellites and adding a global load-order surface. The per-class map in §5 achieves eager validation without it — the source registers *itself on the class* through the `.overrides` include the consumer already opted into, so core never learns about gems and an unincluded adapter stays tolerant by construction.
