...
- (a subfield `default:` beneath the referenced field needs no guard: value-level defaults resolve the child's value on the read path and never synthesize the parent — PRO-2903 — so a wire-omitted referenced field settles nil/falsey exactly as the emitted clause reads it; a subfield `preprocess:` likewise never materializes an absent root);
