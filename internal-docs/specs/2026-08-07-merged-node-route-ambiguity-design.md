...
The emitted property at a merged node is built from `property_representative` (the first non-model config) and its generated `<leaf>_id` from `model_configs.first`, so `of:`, `shape:`, `default:` and the description are all declaration-order-dependent in reflection. That is pre-existing, deliberately specced as an accepted divergence (`property_name_collision_spec.rb:2903`), and orthogonal to value supply — but deleting the two-default rejection widens exposure to it, so it gets its own ticket rather than silence: PRO-3082, which carries the measured `default:` divergence the two-default relaxation newly exposes.

One more hole surfaced while auditing whether that relaxation could let through a contract the rest of the system cannot serve. It cannot — but the audit found that `node_optional?`'s satisfiability credit is per-NODE, so a single defaulted route excuses a required sibling route on the same node that can then never pass. It needs only one default, so this branch neither creates nor widens it, and decision 8's claim that the credit gets more accurate with two defaults should not be read as a claim that it is sound in the mixed case. Filed as PRO-3083.
