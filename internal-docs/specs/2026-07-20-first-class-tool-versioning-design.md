...
A new core module `Axn::Core::Versioning` (sibling of `Axn::Core::SemanticHints`) adds a `tool_version` DSL:
...
- `tool_version N` sets the tool's contract version. Absent ⇒ version 1 and the default.
- `default: true` blesses this version as the movable **stable pin**.
- The registry's identity becomes `(tool_name, tool_version)`. `_assert_unique_tool_names!` relaxes to unique-per-`(tool_name, tool_version)`.
...
- **The core-exposed *designations*** those policies consume are what this PR provides:
  - **`latest`** = computed `max(tool_version)` in a group. Not declared. What MCP/ruby_llm read.
...
### The DSL/reader is named `tool_version`, not `version`

`tool_version` is the sibling-of-`tool_name` name the whole design leans on, and choosing it over a bare `version` buys two things:

- **No shadowing guard.** `include Axn` extends axn's class-method DSLs onto the including class, and Ruby places extended modules *above* the superclass chain in singleton-method lookup — so a DSL named `version` could silently win over a `.version` a base class already owns, or need a `MethodShadowing` guard (raise/defer) to avoid it (the PRO-2875 hazard that `description` navigates). `tool_version` is no more collision-prone than `tool_name` or `semantic_hints`, **neither of which is guarded**, so it follows that precedent: no `MethodShadowing` check, no split reader/setter module, no definition-time raise. A whole decision branch disappears.
- **`version` stays free** for downstream/custom use at zero cost. No current use case; cheap insurance.

`tool_version` is a single method that is both setter and reader (like `semantic_hints`): `tool_version 2, default: true` writes; zero-arg `tool_version` reads the effective value.

### DSL shape: `default:` is a keyword on `tool_version`

`tool_version 2, default: true`. A bare tool (no `tool_version`) is version 1 and the default; standalone `default: true` on version 1 is a harmless no-op (it is already default). `tool_version` is single-valued and **core, not per-adapter** — unlike `tool_name`, there is no per-adapter version override. Grouping still keys on `tool_name(adapter)`, so latest/default resolve per-adapter naturally.

### Validation is fail-loud at declaration; group conflicts fail at enumeration

Anything knowable from a single class raises at declaration: `tool_version` must be an `Integer >= 1`; the mismatch `::Vn` guard below. Anything requiring the whole group (a duplicate `(tool_name, tool_version)`, two `default: true`) is only knowable once every file is loaded, so it raises during group resolution — the same timing as today's `_assert_unique_tool_names!`. **Gaps between versions are legal** (`v1` + `v3`, no `v2`): versions get retired, and requiring contiguity would only punish deletion.

### `tool_name` derivation drops a trailing `::Vn` when `tool_version` is declared (option B)
...
So when a class **declares `tool_version`** *and* its final constant segment matches `/\Av\d+\z/i`, `tool_name` derives from the **enclosing** namespace (`AgentTools::ApproveLoan::V2` → `"approve_loan"`). This is scoped to *name* derivation and gated on an explicit `tool_version` declaration — distinct from the *filename→version* derivation the ticket deliberately rejects (`tool_version N` remains the sole source of truth for the number). Explicit `tool name:` / `axn_name` still win ahead of derivation, unchanged.

Gating on the `tool_version` declaration (rather than the name pattern alone) keeps the name segment and the version number consistent — a `::V2` class is genuinely version 2, never "named V2 but secretly version 1" — and makes each file in a promoted folder carry an explicit, self-documenting `tool_version N`.
...
- **`::V2` present, but declared `tool_version M` with `M != 2`** (`::V2` declaring `tool_version 3`) → raise **at declaration**, inside the `tool_version` DSL call (both the constant name and the declared number are in hand there). Message: align them or rename. Keeps the segment and the number honest, since the segment is what derivation drops.
- **`::Vn` present, no `tool_version` declared** → raise **at enumeration** (during `tool_name` derivation / group resolution). There is no declaration event to hook here — `tool_version` was never called — so this is caught where the derivation would otherwise silently orphan the class (`v1.rb` deriving `..._v1`), the same timing class as the duplicate-`(tool_name, tool_version)` and two-`default:` checks. Message: declare `tool_version N` or rename the constant.

## Behavior

### `Axn::Core::Versioning`

Two `class_attribute`s hold the state: `:_tool_version, instance_accessor: false, default: nil` (`nil` = undeclared ⇒ effective 1 & default) and `:_tool_version_default, default: false`.

`tool_version` is a single setter/reader (no `MethodShadowing` check — see the naming decision):

```ruby
def tool_version(value = NOT_SET, default: false)
  return (_tool_version || 1) if value.equal?(NOT_SET)         # reader → effective
...
Zero-arg `tool_version` returns the effective version (`_tool_version || 1`) and is what the registry, adapters, and reflection consume — the sibling of `tool_name`. `_tool_version` being `nil` vs. set is how the registry and the derivation guards know whether the tool opted into versioning.

The mismatch guard (`::V2` declaring `tool_version 3`) lives in the `tool_version` DSL itself: after setting `_tool_version`, if `self.name`'s final segment matches `/\Av(\d+)\z/i` and the captured number differs from `value`, raise. (An anonymous/unnamed class has no constant segment, so the check is simply skipped — the factory/`Class.new` path is unaffected.) The no-`tool_version` guard cannot live there — `tool_version` is never called — so it lives in the option-B branch of `tool_name` derivation: when the final segment matches `/\Av\d+\z/i` but `_tool_version` is nil, raise instead of deriving. Both reuse the same segment regex.
...
### Registry — identity `(tool_name, tool_version)`
...
- **Repeat `tool_version`.** Last-wins on a single class is acceptable (a scalar attribute, unlike the `tool` DSL's membership); no repeat guard is added.
...
- **Duplicate `(tool_name, tool_version)`.** Raises at group resolution, message names the version.
- **`::Vn` without `tool_version`.** Raises at enumeration (`tool_name` derivation).
- **`::V2` with `tool_version 3`.** Raises at declaration (inside the `tool_version` DSL).
- **A base class defining `.tool_version`.** Not guarded — `tool_version` follows the `tool_name`/`semantic_hints` precedent (unguarded), which is the point of not naming it `version`.

## Acceptance

- `tool_version 2, default: true` sets the effective version to 2 and flags the default; a class with no `tool_version` reads 1 and is its group's default.
- `tool_version 0`, `tool_version -1`, `tool_version "2"` raise at declaration.
- `AgentTools::ApproveLoan::V2` declaring `tool_version 2` derives `tool_name` `"approve_loan"`; `::V2` without `tool_version` raises; `::V2` with `tool_version 3` raises.
- `version` (bare) is untouched by axn — a class may define its own `.version` with no collision.
...
- Two classes with the same `(tool_name, tool_version)` raise; two `default: true` in one group raise.
