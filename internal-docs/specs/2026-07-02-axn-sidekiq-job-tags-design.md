...
3. Run **inbound preprocessing + inbound defaults only** — `apply_inbound_preprocessing!`, `apply_defaults!(:inbound)` — wrapped in `PipingError.swallow`, so an invalid-input enqueue still succeeds (we resolve best-effort against whatever coerced cleanly). This applies `preprocess:` transforms and inbound `default:`s before resolution — the one thing this pass buys over a bare instance. **Inbound validation is deliberately NOT run here**: validation only checks, it doesn't transform anything a facet resolver reads, so running it would add cost for no benefit — including forcing a read of every validated field (a `model:` field's `.find`, synchronously, at enqueue) and running any user `validate:` procs. `model:` records resolve **lazily on read** (facade.rb), so a model-derived facet triggers its lookup only if a resolver actually reads it — not for every enqueue.
4. Resolve only the **`from: :inputs`** facets for the enabled sources, via the memoized `resolved_input_tags` / `resolved_input_dimensions` readers (both delegate to `Core::Tagging.resolve(facets, action:, from: :inputs)`, which resolves each input-phase resolver independently and drops per-facet errors / `nil` results). **`from: :result` facets are excluded by construction** — they can't resolve before the body runs, and the phase filter never touches them. (This uses the facet-phase system from PRO-2854/#142; before that landed, this pass resolved everything and relied on result-phase resolvers self-omitting via `nil`/raise — the explicit phase filter is strictly cleaner.)

The seam lives on `Axn::Executor` (which owns the inbound coercion methods and the memoized `resolved_input_*` phase readers) as a new entry point — `#resolve_inbound_facets(sources)`. The Sidekiq-specific formatting and attachment live in the adapter.
