...
Nothing here breaks a sibling gem's *code*. Grepping executable Ruby across all six gems and three apps for each moved or renamed constant — `DuplicateFieldError`, `UnreraisableException`, `StrategyNotFound`, `DuplicateStrategyError`, `Async::AdapterNotFound`, `Async::DuplicateAdapterError`, `Mountable::MountingTypeNotFound`, `Mountable::DuplicateMountingTypeError` — returns **zero references to all eight**, and no gem reaches into a moved `Internal::Reflection` module either. The port work is additive, plus one pre-existing break this audit found:

- Every gem: add `include Axn::Error` to its base error class (axn-mcp needs the base first).
- axn-openapi's three references to `Axn::Reflection::UnserializableValue` (`openapi.rb:29`, `dispatcher.rb:95`, `dispatcher.rb:120`, in both its main checkout and the `clean-serialization` worktree) are all inside comments; the live handler rescues `StandardError, SystemStackError` and never names the constant. PRO-2992 already moved it to `Extensions::Serialization::UnserializableValue`, so the comments are stale prose rather than a break — worth a follow-up edit so they don't mislead the next reader, but the gem is not broken on its next bump.
