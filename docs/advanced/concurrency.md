...
**Manually driven fibers are not supported.** Resuming fibers by hand (`Fiber.new` / `#resume` with no scheduler) under the default `:thread` isolation shares one nesting stack across them; when Axn detects two call trees interleaving on it, it repairs the stack and logs a one-time warning, but log prefixes and exception attribution are wrong while they overlap. An interleave in which one fiber's action runs and finishes entirely while another is suspended is not detected. Run such fibers under a Fiber scheduler with `isolation_level = :fiber` instead.
