...
If the orchestrating action declares its own base `error "…"`, that base is prepended to the step failure (on top of the step prefix), so a failed step renders as `"Parent base: validation: Input is invalid"`. See [base/reason attachment](/usage/writing#prefixing-failure-reasons) for how the cascade composes.
