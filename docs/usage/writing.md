...
1. **Callbacks observe every settled call.** `on_success`/`on_failure`/`on_exception`/`on_error` fire whenever a call settles, from any origin — including an inbound validation failure, where no hook ran. They are the seam for "must not miss a call", within four limits:
   ...
   - In development with [`best_effort_raises_in_dev`](/reference/configuration#best-effort-raises-in-dev) on, anything axn would otherwise swallow during settlement — a callback's raise, or an `error`/`success` message callable resolved while settling — is re-raised instead, and where it lands depends on when. During failure or exception settlement (a message callable, or an `on_error`/`on_failure`/`on_exception` callback) it raises out of `.call`, so the callbacks not yet run never fire; a raising `error` message fires none of them. An inline `on_success` raises inside the call, which re-settles it by the kind of halt: a raise as an exception (firing `on_error`/`on_exception`), a `fail!` as a failure (firing `on_error`/`on_failure`). A transaction-deferred `on_success` runs at commit, after `.call` has returned. Test and production always swallow these `StandardError`s and carry on.
