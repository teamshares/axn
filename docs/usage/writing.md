...
1. **Callbacks observe every settled call.** `on_success`/`on_failure`/`on_exception`/`on_error` fire whenever a call settles, from any origin — including an inbound validation failure, where no hook ran. They are the seam for "must not miss a call", within four limits:
   - An exception axn does not capture, raised before the call settles, means it never settles, so no callback fires. Raised *after* settlement — by a callback, or by a message callable read during settlement — it still escapes `.call`, and the callbacks after it never fire, although the result had already settled.
