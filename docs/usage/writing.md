...
Both of these optionally accept `type:`, `optional:`, `allow_nil:`, `allow_blank:`, `allow_empty:`, and any other ActiveModel validation (see: [reference](/reference/class)).
...
- A `done!` raised by contract resolution itself (a `default:`, `preprocess:`, or a callable a validation evaluates) skips outbound validation — see [What runs when](#what-runs-when)
...
::: info Works at any depth (subfields and parents alike)
`user_facing:` works at any depth: declare it on a [subfield](/reference/class#nested-subfield-expectations) and that subfield's violation settles user-facing by its own declaration, or on a parent that has subfields — when the parent itself fails, its stranded descendants' checks are attributed to the parent (they could never pass with a nil/invalid parent) instead of paging over its user-facing message. The settling rule is unchanged: any dev-facing violation anywhere dominates a mixed failure and the call still pages. A shape block's member checks default dev-facing, but a member may itself opt into `user_facing:` with full parity to a field (`true`/String/Symbol/Proc), surfacing that member's own failure to the caller; a shape-carrying field's own errors (its presence/type check) honor the field's `user_facing:` independently of its members. Model-consistency checks and ambient_context subfields stay always dev-facing (framework-supplied — there is no user to face). When a *specific* nested check should face the user, declare that leaf as its own subfield instead: `expects :email, on: :profile, user_facing: "Please provide an email"`.
:::
...
² Outbound resolution still runs after a call that returns, and after a `done!` from `call` or a hook, so an unset required exposure turns either into an `exception` (`OutboundValidationError`). A `done!` raised by contract resolution itself is the exception: that is, a `done!` from any `preprocess:`, `default:`, or callable a validation evaluates that runs outside the hook chain — during inbound validation, in an `exposes` `default:` or validation, or during the outbound copy-forward of a field both expected and exposed that nothing read earlier. It settles as success immediately, so outbound validation does not run (and neither do any outbound defaults not yet applied), even with a required exposure unset. That holds on the normal path only: once the call is already settling a raise or `fail!`, outbound defaults are applied best-effort, so a `done!` from one is swallowed and the original halt stands.
...
1. **Callbacks observe every settled call.** `on_success`/`on_failure`/`on_exception`/`on_error` fire whenever a call settles, from any origin — including an inbound validation failure, where no hook ran — subject to each callback's own exception-class filter and `if:`/`unless:` condition ([Callbacks](/reference/class#callbacks)). They are the seam for "must not miss a call", within these limits:
   ...
   - In development with [`best_effort_raises_in_dev`](/reference/configuration#best-effort-raises-in-dev) on, anything axn would otherwise swallow — a callback's raise, or a message callable's — is re-raised instead, and where it lands depends on when it runs. During failure or exception settlement (the `error` message, or an `on_error`/`on_failure`/`on_exception` callback) it raises out of `.call`, so the callbacks not yet run never fire; a raising `error` message fires none of them. The `success` message resolves lazily, so it raises wherever it is first read: from `result.success` after `.call` has returned, or inside the call if an inline `on_success` reads it, which re-settles the call as below. An inline `on_success` raises inside the call, which re-settles it by the kind of halt: a raise as an exception (firing `on_error`/`on_exception`), a `fail!` as a failure (firing `on_error`/`on_failure`). A transaction-deferred `on_success` runs at commit, after `.call` has returned. Test and production always swallow these `StandardError`s and carry on.
