...
1. **Callbacks observe every settled call.** `on_success`/`on_failure`/`on_exception`/`on_error` fire whenever a call settles, from any origin — including an inbound validation failure, where no hook ran. They are the seam for "must not miss a call", within four limits:
   ...
   - In development with [`best_effort_raises_in_dev`](/reference/configuration#best-effort-raises-in-dev) on, a callback that raises is re-raised out of `.call` instead of swallowed, so the callbacks after it never fire — a broken `on_error` stops `on_failure`/`on_exception` too. Test and production always swallow it and carry on.
