...
- a value at a position whose `output_schema` was reflected from a Data/Struct's own members — a `shape:` block, or a bare `of:` element/values type — but which serializes through a projection *other than* the declared class's: a runtime subclass's own `as_json`/`to_h`, one contributed by an included module, or a singleton method. The class-level schema and the instance-level render would then actively disagree (e.g. a required member silently missing from the body), which is worse than merely ugly. This is an *ownership* check — it fires whenever the value's own table picks a different projection than the declared class's did, whether or not that specific override happens to still satisfy the position's own requirements — so if you see it for a value you know renders fine, that's the signal: define the projection on the declared class itself (making the position honestly opaque, the same as the next case), or expose a type that doesn't carry the field you meant to hide;
...
**Known gap, in a Rails app only:** a Data/Struct value nested *directly* inside another Data/Struct (not through a Hash or an Array — those ARE covered) is rendered by ActiveSupport's own `Data#as_json`/`Struct#as_json` (`to_h.as_json`) in one step, so a displacing override on the *inner* value is never reached by this check. Outside Rails, and everywhere else inside it, the check applies at every depth.
