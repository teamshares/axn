...
These rules keep adapters interoperable:
...
- **Reflection is best-effort and biased *stricter* than runtime** — a call that follows the schema normally passes runtime contract validation, but the bias is not a guarantee, and every exception is documented rather than hidden. Documented exceptions include an invalid literal `default:` (`type: :uuid, default: "nope"`) reflects as optional though the omitted call fails at runtime; and any property carrying an `Additional constraints apply that JSON Schema cannot express:` `description` (next bullet) is enforced more tightly than it is stated, so a schema-valid value may still be rejected there. Surface these caveats to your users; don't try to fight them in the adapter. (A deep subfield under a `model:`/non-object parent has no JSON representation and is omitted with a `logger.warn` — pass it through, don't paper over it.)
- **Where a collision projection can't state a constraint, it says so in that property's `description`** — pass this text through, including descriptions nested in schema branches. The sentence begins `Additional constraints apply that JSON Schema cannot express:` and describes checks that still run. Sources include conditional validators at colliding declarations (the check applies only when its gate opens), checks on transformed values or their descendants (`coerce:`/`preprocess:` make the checked value differ from the wire input), and checks without an equivalent keyword on a surviving JSON type (such as numeric bounds on strings or Ruby's whitespace-aware string blankness). The report renders a schema fragment or validator options as JSON; it is explanatory text, not an executable schema. A schema-valid value may still fail these runtime checks. The class also logs deduplicated warnings naming the affected positions for the author.
- **An input property may carry `allOf`** — not only at the schema root, where a conditionally-required field's clause has always lived. A `shape:` member and a subfield node that both bind one wire position are two separate declarations the runtime enforces together, and where the emitter can't union their keywords into one node (a member whose type has no object branch, or a name two declarations both claim), it states the conjunction honestly as a sibling `allOf` branch instead of picking a winner. Don't assume a property is a flat `type`/`properties` node — walk `allOf` (and `anyOf`) the same way you already have to.
...
It raises `Axn::Extensions::Serialization::UnserializableValue` (an `ArgumentError`) when an exposed value has no honest JSON representation, naming the path to it (`records[3].price`, not "something"). Six cases raise always, because the body would be *wrong* — or not JSON at all:
...
- a value at a position whose `output_schema` was reflected from a Data/Struct's own members — a `shape:` block, or a bare `of:` element/values type — but which serializes through a projection *other than* the declared class's: a runtime subclass's own `as_json`/`to_h`, one contributed by an included module, or a singleton method. The class-level schema and the instance-level render would then actively disagree (e.g. a required member silently missing from the body), which is worse than merely ugly. This is an *ownership* check — it fires whenever the value's own table picks a different projection than the declared class's did, whether or not that specific override happens to still satisfy the position's own requirements — so if you see it for a value you know renders fine, that's the signal: define the projection on the declared class itself instead — a class that renders itself this way is honestly opaque (`output_schema` reflects `{}` there, the same as any class with its own `as_json`/`to_h`) rather than asserting a shape it might not produce — or expose a type that doesn't carry the field you meant to hide;
...
**Known gap, wherever ActiveSupport's `json` core_ext is loaded** (every Rails app has it; a non-Rails process can too — `require "globalid"`, among others, pulls it in as a side effect): anything nested *inside* a Data/Struct value, at any depth — a member that is itself a Data/Struct, or one further inside a Hash or an Array that member holds — is rendered by ActiveSupport's own `Data#as_json`/`Struct#as_json` (`to_h.as_json`) in one step, so a displacing override anywhere in that subtree is never reached by this check. A Hash or an Array *at the top level of an exposure* is still walked and checked normally; the gap is specifically what ActiveSupport renders in one call once a Data/Struct value is reached. Without that core_ext loaded, the check applies at every depth with no such gap.
