...
When a class declares `tool_version` and its constant ends in a `::Vn` segment, `tool_name` derives from the enclosing namespace (`AgentTools::ApproveLoan::V2` → `"approve_loan"`), so both versions group. The promotion moves the Ruby constant but not the wire contract — identity stays `tool_name`. Two guardrails keep the convention honest: `Axn.tools_for` raises when it enumerates tools if a member's `::Vn` constant declares no `tool_version` (so a promoted `v1.rb` can't silently orphan itself) — this comprehensive check is `tools_for`'s alone; `Axn.versions_for` is a post-enumeration lookup that relies on it, and `tool_name` derivation itself is a pure reader that does not raise — and a `::V2` constant declaring a different number (`tool_version 3`) raises at declaration.
