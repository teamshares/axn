...
- **The schema is exact at its core and never stricter than runtime elsewhere** ([what it promises](/reference/class#what-the-schema-promises)). Types, requiredness, nullability, nesting, literal enums and literal bounds are exact in both directions; everything else — a `format:` regex with no faithful ECMA spelling, an exclusion set, a gated check, a check on a transformed value — is emitted only where it is exact, and otherwise left out and named. So a call that follows the schema is never refused by the schema's own strictness, but may still be rejected at runtime exactly where the schema says it cannot state a constraint. Surface these caveats to your users; don't try to fight them in the adapter. (A deep subfield under a `model:`/non-object parent has no JSON representation and is omitted with a `logger.warn` — pass it through, don't paper over it.)
- **What the schema cannot state, it names** — in that property's `description`, as a sentence beginning `Additional constraints apply that JSON Schema cannot express:`, and structurally through `klass.input_schema_residues` (each with the `path` of property keys, the `summary`, and a `kind` of `:conditional`, `:inherent` or `:unfixed`). Pass the description text through, including descriptions nested in schema branches; read `input_schema_residues` when your transport wants the gaps somewhere else (a tool-level note, a warning at registration). `input_schema_residues` is defined even when your adapter base owns `input_schema`, and reports axn's projection. The report renders a schema fragment or validator options as JSON; it is explanatory text, not an executable schema. Only `:unfixed` gaps are also logged for the author, once per class.
