...
- **Reflection is best-effort and biased *stricter* than runtime** — a call that follows the schema is normally not schema-rejected, but the bias is not a guarantee, and every exception is documented rather than hidden. Two of them: an invalid literal `default:` (`type: :uuid, default: "nope"`) reflects as optional though the omitted call fails at runtime; and any property carrying an `Additional constraints apply that JSON Schema cannot express:` `description` (next bullet) is enforced more tightly than it is stated, so a schema-valid value may still be rejected there. Surface these caveats to your users; don't try to fight them in the adapter. (A deep subfield under a `model:`/non-object parent has no JSON representation and is omitted with a `logger.warn` — pass it through, don't paper over it.)
- **Where the document can't state a constraint, it says so in that property's `description`** — and you should pass that text through, not strip it. The sentence begins `Additional constraints apply that JSON Schema cannot express:` and names what still runs, rendering the constraint it couldn't state as JSON so a model choosing an argument can act on it: `… ({"type":"integer","const":5})`. Two positions produce one today — a value that is transformed (`coerce:`/`preprocess:`) before its own checks run, so the keywords describe the transform's output while the wire carries its input; and a declared type JSON has no form for (`type: Object`), whose string-shaped keywords are inert beside the real type. Both are **looser** than the runtime at that position, which is why they're stated rather than silently narrowed: a call that satisfies the schema there may still be rejected, and the description is what tells the caller so. The class also logs one warning naming every such position, for the author rather than the caller.
