...
1. **Is this job/notice Axn-owned?** Ask `Axn::Async.owned_by?(signal)`. It accepts a resolved Class, a String class name (including the ActiveJob adapter's `"::ActiveJobProxy"` suffix), or a raw Sidekiq job Hash (string or symbol keys), and folds in every detail of Axn's async wiring — the generic Sidekiq worker, the proxy naming convention, and the `display_class` wire format. Pass it whatever your error reporter's plugin hands you; blank/unrecognized input returns `false` without raising.
...
Honeybadger.configure do |config|
  config.before_notify do |notice|
    ...
    # Axn's on_exception is handling reporting for these. `owned_by?` recognizes each
    ...
    next notice.halt! if signals.any? { |signal| Axn::Async.owned_by?(signal) } # [!code focus]
  end
end
```
