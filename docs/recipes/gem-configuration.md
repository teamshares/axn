...
| `validate:` | A callable checking the assigned value. Returning a truthy non-String value passes. Returning `false` or `nil` raises `ArgumentError` with a generic message. Returning a String also raises `ArgumentError`, but uses that String as the reason — worth it for a setting whose value is an object you supply, where "invalid" alone doesn't hint at the contract. The callable may instead raise its own `ArgumentError` for a fully custom message. |
