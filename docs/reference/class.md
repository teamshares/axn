...
Two things differ from its mirror, both following from the inversion. Tolerance is not consulted at all: a tolerated `nil` is one more value that **passes**, and no amount of passing makes a check something can fail, so `optional:`/`allow_nil:`/`allow_blank:` do not rescue one. And a gate does not either — a gate can only remove the check, so closed it enforces nothing and open it enforces nothing.
