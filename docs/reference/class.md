...
On **output** neither is emitted. There the schema describes what the action *produces*, so a narrowing rejects values axn successfully serialized — the same direction a self-gated validator is reduced away for. Only an exactly-translated pattern reflects outbound.
