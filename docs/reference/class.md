...
A separate bound guards *size* rather than naming: a `shape:` graph that shares one nested shape object between **sibling** members multiplies out (N levels of two-way sharing name 2^N properties), so one naming more than 25,000 properties is rejected at class definition — copying it to store the contract is itself the cost. A contract whose declared shapes merely *sum* past that bound is rejected when a projection is first built, since only the total reveals it.
