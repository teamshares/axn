...
* `model: true` (or `model: TheModelClass` or `model: { klass: TheModelClass, finder: :find }`) - allows auto-hydrating a record when only given its ID
  * Example:
    ...
    **When no record is found.** A finder has two ways of saying "no such record": returning `nil`, or raising its own not-found error (`ActiveRecord::RecordNotFound`, for the default `:find`). Axn treats both as the same outcome — an inbound contract violation reading `User not found`, distinct from the `User can't be blank` you get when no `<field>_id` was supplied at all.
    ...
    `not_found_on:` names the exception classes whose being raised means "no such record" — decided by the raised class's ancestry, so a subclass of a declared class counts too. It **replaces** the default rather than adding to it, so a non-ActiveRecord finder can opt out of `RecordNotFound` entirely. Every entry must be an exception class; anything else (including `nil`) raises `ArgumentError` at declaration, since a value that can never be raised would silently disable the handling rather than extend it. Outside Rails, where `ActiveRecord::RecordNotFound` is undefined, the default set is empty and a finder that returns `nil` is the only miss.
