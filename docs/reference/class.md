...
It stands down wherever a value could still pass or the literals cannot be judged at declaration: a tolerance flag (`optional:`/`allow_nil:`, under which nil passes), an undeclared or pseudo-type `type:`, a set read dynamically (a Symbol, a Proc, an `ActiveRecord::Relation`) or held in an `Array` subclass, a `Hash` `accept:` set, a `comparison:` bound ActiveModel resolves per call, and a literal that compares across classes anyway — `type: Float, comparison: { greater_than: 0 }` is an Integer bound on a Float field and compares perfectly well. An `if:`/`unless:` gate does **not** excuse either refusal: a gate decides whether a check runs, not what it can mean.
