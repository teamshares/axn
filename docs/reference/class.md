...
* `of:` - names what is INSIDE a container: for `type: Array`, each element; for `type: Hash`, its keys and/or values
  ...
  * A Hash's `of: { values: Bar }` reflects in the schema as `additionalProperties: <Bar's schema>` (a union `values:` emits `anyOf` branches, a `Data` `values:` class emits its members as `properties`); `keys:` emits nothing at all — every JSON object key is already a string, so `keys: String` would say nothing actionable and `keys: Symbol` would misdescribe the wire format
  * The `of:` option bag itself only accepts the keys its container allows (`klass:`/`message:` plus the shared ActiveModel options for an Array; `keys:`/`values:` plus the shared options for a Hash) — anything else, including a nested `of:` or a nested `shape:`, raises `ArgumentError` at declaration time. `of:` alongside `shape:` on a Hash, and a subfield (`on:`) rooted at a map (at any depth), are both refused as "not supported yet"
