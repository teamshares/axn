...
A declaration whose admissible **sizes** form an empty interval is refused on the same terms — a floor it imposes sitting above a ceiling it also imposes, so nothing can satisfy it. Four spellings reach it: `absence:` on a typed field (the non-emptiness floor a typed field carries rejects the only value `absence:` admits), a `length:` ceiling of 0 against that same floor, a `length:` whose own `minimum:` sits above its own `maximum:`, and an `inclusion:` set whose every member is outside the admitted sizes (`type: Array, inclusion: { in: [[]] }`). Each emitted a document no caller could satisfy — `{minItems: 1, maxItems: 0}`, or an `enum` naming nothing the size bounds allow. To declare "the empty container, and nothing else", drop the floor with `allow_empty: true` (or `presence: false`); the ceiling then reflects as `maxItems: 0`. As above, a nil tolerance stands the refusal down, and a set the guard cannot read (a Symbol, a Proc, a `Range`, an `Array` subclass) or a member whose `size` is not Ruby's own leaves the `inclusion:` branch silent.

An `if:`/`unless:` gate does **not** excuse any of these refusals: a gate decides whether a check runs, not what it can mean — and reflection is static-maximal, so a gated bound is emitted regardless, leaving the document unsatisfiable even where the runtime is not.
...
A field that rejects empty reflects that into its schema as `minItems` / `minProperties` / `minLength`, and a declared ceiling reflects as `maxItems` / `maxProperties` / `maxLength` (an exact `length: { is: 2 }` emitting both). Two spellings name a ceiling: `length:`, and `absence:`, which rejects every non-blank value and so names size 0 as the only admissible size — `expects :tags, type: Array, absence: true, allow_empty: true` emits `maxItems: 0`. On a `String` that ceiling is biased stricter than the runtime rather than exact, since `blank?` covers a whitespace-only value that `empty?` does not — the mirror of the floor `presence:` emits, which `minLength` cannot state exactly either. A bound ActiveModel resolves per call (a Symbol or Proc) and an infinite one emit nothing, since no fixed number expresses them.
