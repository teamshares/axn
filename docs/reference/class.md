...
* `model: true` (or `model: TheModelClass` or `model: { klass: TheModelClass, finder: :find }`) - allows auto-hydrating a record when only given its ID
  * Example:
    ...
    **The `<field>_id` wire key is a lookup scalar.** Because the id key holds the token the finder consumes, nothing else may claim it as a nested object. A dotted `on:` whose intermediate segment is spelled `<field>_id`, an explicit `<field>_id` that a subfield then nests under, or a `shape:` member of that name carrying its own `members:` all raise `ArgumentError` at declaration: one wire key cannot be both a lookup token and an object parent, so the reflected schema could only emit one of the two and which one survived followed declaration order. A plain scalar `<field>_id` sibling is untouched — that is the supported spelling, and the one that supplies the token. So is a **conditional** member that reflection omits anyway: where an explicit intermediate subfield keeps the member out of the emitted schema, a member carrying its own `if:`/`unless:` — or nested under one that does — claims the key on no call at all, so the emitted id and the runtime agree and the declaration stands.
