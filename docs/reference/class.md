...
* `validate: [callable]` - Support custom validations (fails if any string is returned OR if it raises an exception)
  * Example:
    ...
    ```
  * The bag form (`validate: { with: <callable> }`) only accepts `with:` plus the shared ActiveModel options — a key it does not know raises `ArgumentError` at declaration. `message:` is accepted for parity with other bags but never read: the callable's own return value **is** the error message.
