...
It stands down wherever a value could still pass or the literals cannot be judged at declaration: a tolerance flag (`optional:`/`allow_nil:`, under which nil passes), an undeclared or pseudo-type `type:`, a set read dynamically (a Symbol, a Proc, an `ActiveRecord::Relation`) or held in an `Array` subclass, a `Hash` `accept:` set, and a `comparison:` bound ActiveModel resolves per call.

It stands down on a literal of another class that would satisfy the declaration anyway — one whose class is a content-comparing root the declared type descends from (a plain-`Array` literal against an `Array` subclass, since `SubArray.new([1]) == [1]`), and one in the same cross-comparable family as the declared type: every `Numeric` with every other, and the `Date`/`Time`/`DateTime` trio. `type: Float, comparison: { greater_than: 0 }` is an Integer bound on a Float field and compares perfectly well.
