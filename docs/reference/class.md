...
* `confirmation: true` - declares a companion input, `<field>_confirmation`, and fails unless it matches the field's actual value
  * Declaring `confirmation: true` on `:password` implicitly adds `password_confirmation` to the contract — you don't write a separate `expects` for it yourself. An explicit `expects :password_confirmation, ...` alongside the base wins over the implicit one, so you can still give the companion its own `type:`/`optional:`/etc. by declaring it directly; the implicit version only fills in when you haven't.
  * The companion inherits the base field's `type:`, `coerce:`, `preprocess:`, `method_call:`, `user_facing:`, and `sensitive:` — so redaction and caller-facing error handling apply to `password_confirmation` exactly as they apply to `password`. It does **not** inherit a `shape:` block, `of:`, or `inclusion:` — the companion's own contract is only the base's `type:`, so its emitted schema carries none of those constraints. This is harmless rather than a gap: the companion is compared for equality against whatever the base actually holds, and a value equal to one that already satisfies the base's shape/`of:`/`inclusion:` constraints satisfies them too — the equality check subsumes them. It never inherits `default:`: a defaulted companion would produce and then match its own value on any call the caller leaves it out of, quietly passing a confirmation nobody actually supplied.
  * The companion is required exactly when the base field is present: send `password` without `password_confirmation` and the call fails; send neither and it passes — there's nothing to confirm. A companion you *do* supply is still compared against whatever the base actually holds, whether the base itself is required, optional, blank-tolerant, or nil-tolerant — a blank or `nil` base is a value a mismatched companion can still contradict. A base carrying a `default:` is always present, so its companion is always required too — and since `default:` isn't inherited (above), the caller has no way to see the default's value in the schema; the only passing call supplies `<field>_confirmation` equal to whatever the default actually is. An `if:`/`unless:` gates the requirement along with the comparison it serves, whether you write it on the declaration (`expects :password, type: String, confirmation: true, if: :admin`) or on the `confirmation:` entry itself (`confirmation: { if: :admin }`) — on a call where the gate is closed nothing compares the pair, so nothing demands the companion either. An `allow_blank:` on the `confirmation:` entry (`confirmation: { allow_blank: true }`) does the same for a blank base: ActiveModel skips the comparison there, so no companion is demanded for it.
  * The companion also gets a reader — `password_confirmation`, or `<base reader>_confirmation` when the base is aliased (`expects :password, as: :pw, confirmation: true` reads as `pw_confirmation`). That reader is inferred rather than declared, so it defers: if you wrote a method of that name, or another declaration's `as:` already claims it, yours stands and the companion simply goes without a reader. The companion is still validated, redacted and reflected either way: without a reader of its own it is enforced against the wire value directly, so a method you wrote can never stand in for the input the pair requires. A subfield declared `on:` that companion reads the wire value too, for the same reason — unless the name belongs to another declaration, in which case `on:` reads that declaration's value, because that is the value the reader answers with.
  * Not supported on `exposes` (there's no caller input to confirm an output against) or inside a shape block's `field` (a member has no reader for a companion to attach to) — both raise at declaration.
  * This is a deliberate departure from ActiveModel's own `confirmation:`, which skips the comparison whenever the confirmation accessor reads `nil` — an omitted `password_confirmation` silently passes there. That default fits ActiveModel's use case, not axn's: its confirmation attribute is a virtual accessor on a persistent record, and it reads `nil` on every save that doesn't touch the field, so failing on `nil` there would break `user.update(name: "x")` until someone re-typed the password — which is exactly why [the Rails guides](https://guides.rubyonrails.org/active_record_validations.html#confirmation) tell you to bolt on a `presence` check by hand. An axn call carries no such history: it's one inbound message, validated once, never a record revisited across unrelated updates — so `confirmation:` bakes the presence check in rather than leaving it as a footnote you have to add yourself.
