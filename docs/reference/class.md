...
A declared `numericality:` or `comparison:` bound reflects the same way: `greater_than` as `exclusiveMinimum`, `greater_than_or_equal_to` as `minimum`, `less_than` as `exclusiveMaximum`, `less_than_or_equal_to` as `maximum`, `equal_to` as `const`, and a `numericality: { in: 1..10 }` range as both bounds. Where two of them bound the same side — two validators declaring `greater_than`, or an `in:` range beside an explicit `greater_than_or_equal_to` — the emitted bound is the **tighter** one, since ActiveModel enforces both. Two `equal_to:` values that disagree emit `enum: []` — a node no value satisfies, which is exactly what the contract is. `other_than:`, `odd:`/`even:`, a per-call Symbol/Proc bound, a non-finite bound, and a non-numeric `comparison:` bound emit nothing — no keyword says what they mean.
...
Two narrowings worth knowing, both **input-only**. ECMA's `.` excludes `\r` and U+2028/9 where Ruby's excludes only `\n`; and a Ruby `^`/`$` is a **line** anchor where ECMA's, with no flag available, is an **input** anchor (that spelling needs ActiveModel's `multiline: true` to run at all — without it `format: { with: /^\d+$/ }` raises on every call, so it is not a usable declaration). Both emit a pattern accepting a subset of what the runtime does, which is the direction reflection is documented to err in on input.

On **output** neither is emitted, and nor is anything else merely stricter. There the schema describes what the action *produces*, so a narrowing rejects values axn successfully serialized — the same direction a self-gated validator is reduced away for. A flagless `pattern` also counts UTF-16 **code units** where Ruby counts characters, so anything able to match a character outside the BMP is narrower in ECMA: `"😀"` is one character to Ruby and a surrogate pair to ECMA, so `^\D$` matches it and `^\D$` as a pattern does not. The complements (`\D`, `\W`), a negated class (`[^…]`) and a literal astral character therefore stand down outbound too — while `\d`/`\w`, which are ASCII-only and can never match one, and a BMP character like `é`, which is one code unit either way, still reflect. Only an exactly-translated pattern reflects outbound.
