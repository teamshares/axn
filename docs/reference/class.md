...
* **The graph you declared**, at class definition: a `shape:` graph may have at most 25,000 member **paths**. A nested shape object shared between **sibling** members multiplies out, so N levels of two-way sharing are 2^N paths — and the walks that read a stored contract on every logged call (the `sensitive:` redaction set and the sensitive-member test) pay one step per path, with no per-reference memo. Measured: 26,000 paths cost ≈0.5s per call, 262,000 paths ≈4s per call. Shallow sharing is unaffected.
* **The schema it would emit**, when a projection is first built: at most 25,000 JSON properties. What counts is derived from the same plan the emitter acts on, so a shape whose members never become properties costs nothing here — a scalar `of:` (`of: String` with `field :length`) validates members off an element that stays a string, and an outbound-gated or non-member-keyed value emits no object at all. An `of:` element type's own members *do* reach `items`, so they still count, as does a contract whose fields merely **sum** past the bound (only the total reveals that one).
