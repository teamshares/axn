...
* A member is read off the element by declared data only — a Hash key, or a `Struct`/`OpenStruct`/`Data` member (`Data` via `#to_h`, so no method is ever invoked). Reading a member off a non-`Data` object (a reader) or an Array (an Array method) invokes a method, so — like a subfield's [`method_call:`](#resolving-a-subfield-by-calling-a-method-method_call) — it's opt-in: `field :status, type: String, method_call: true`. Without the flag, reaching such a member raises `Axn::ContractViolation::MethodCallNotPermittedError` (loud, never silent — no method runs, so a mutating one like `field :pop` never mutates during validation). The rule applies at each depth of a nested shape.
