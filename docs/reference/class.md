...
* `confirmation: true` - declares a companion input, `<field>_confirmation`, and fails unless it matches the field's actual value
  ...
  * The companion inherits the base field's `type:`, `coerce:`, `preprocess:`, `method_call:`, and `sensitive:` — whatever shape and redaction rules apply to `password` apply to `password_confirmation` too. It never inherits `default:`: a defaulted companion would produce and then match its own value on any call the caller leaves it out of, quietly passing a confirmation nobody actually supplied.
